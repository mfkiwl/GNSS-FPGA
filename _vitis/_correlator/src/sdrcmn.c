...
extern void cpxfft(cpx_t *cpx, int n)
{
    ...
    kiss_fft_cpx *kcpx = (kiss_fft_cpx*) cpx;

    kiss_fft(cfg, kcpx, kcpx);
...
*-----------------------------------------------------------------------------*/
extern double mixcarr(short int *data, int dtype, double ti, int n,
...
*-----------------------------------------------------------------------------*/
extern void correlator(short int *data, int dtype, double ti, int n,
...
#elif defined(PSPL)
    pl_dot_23(dataI,dataQ,code,code-s[0],code+s[0],n,II,QQ);
#endif
    for (i=1;i<ns;i++) {
#if defined(PS)
    	dot_22(dataI,dataQ,code-s[i],code+s[i],n,II+1+i*2,QQ+1+i*2);
#elif defined(PSPL)
        pl_dot_22(dataI,dataQ,code-s[i],code+s[i],n,II+1+i*2,QQ+1+i*2);
#endif
    }
    for (i=0;i<1+2*ns;i++) {
        II[i]*=CSCALE;
...
*-----------------------------------------------------------------------------*/
extern void pcorrelator(short int *data, int dtype, double ti, int n,
